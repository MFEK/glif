...
enum GuidelineType { Horizontal, Vertical }
...
fn draw_guideline(color: Color, where_: f32, gtype: GuidelineType, canvas: &mut Canvas) {
    ...
    match gtype {
        ...
        GuidelineType::Horizontal => {
            ...
            path.line_to((state.with(|v|(v.borrow().winsize.width as f32*((1./factor)))+(-(v.borrow().offset.0*(1./factor)))), where_));
        }
    }
    path.close();
...
pub fn draw_lbearing(canvas: &mut Canvas) {
    draw_guideline(Color::from(LBEARING_STROKE), 0., GuidelineType::Vertical, canvas);
}

pub fn draw_rbearing(width: u64, canvas: &mut Canvas) {
    draw_guideline(Color::from(RBEARING_STROKE), width as f32, GuidelineType::Vertical, canvas);
}

pub fn draw_baseline(canvas: &mut Canvas) {
    draw_guideline(Color::from(LBEARING_STROKE), calc_y(0.), GuidelineType::Horizontal, canvas);
}
