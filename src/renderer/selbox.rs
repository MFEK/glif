// Draw selection box
use skia_safe::{Canvas, Contains, Paint, PaintStyle, Path, Point, Rect};
...
pub fn draw_selbox(canvas: &mut Canvas, v: &RefCell<State>) -> Rect {
    let c1 = v.borrow().corner_one.unwrap_or(PhysicalPosition{x: 0., y: 0.});
    let c2 = v.borrow().corner_two.unwrap_or(PhysicalPosition{x: 0., y: 0.});

    let mut path = Path::new();
    let mut paint = Paint::default();
    let rect = Rect::from_point_and_size((c1.x as f32, c1.y as f32), ((c2.x - c1.x) as f32, (c2.y - c1.y) as f32));
...
pub fn build_sel_vec_from_rect(rect: Rect, outline: Option<&Vec<glifparser::Contour>>) -> Vec<glifparser::Point> {
