use skia_safe::{Canvas, ContourMeasureIter, Paint, PaintStyle, Path, Point, Rect, Vector, Matrix};
...
fn draw_round_point(at: (f32, f32), kind: UIPointType, selected: bool, canvas: &mut Canvas, paint: &mut Paint) {
    let stroke = if selected { SELECTED_STROKE } else { if kind != UIPointType::Point { HANDLE_STROKE } else { POINT_TWO_STROKE } };
    let fill = if selected { SELECTED_FILL } else { if kind != UIPointType::Point { HANDLE_FILL } else { POINT_TWO_FILL } };
    let factor = state.with(|v|v.borrow().factor);
    let radius = POINT_RADIUS * (1./factor) * if selected && kind == UIPointType::Point { 2. } else { 1. };
...
fn draw_square_point(at: (f32, f32), kind: UIPointType, selected: bool, stroke: u32, fill: u32, canvas: &mut Canvas, paint: &mut Paint) {
    let factor = state.with(|v|v.borrow().factor);
    let radius = (POINT_RADIUS * (1./factor))*2. * if selected { 2. } else { 1. };

    let mut path = Path::new();
    match kind {
        UIPointType::Point => {paint.set_color(fill);},
        _ => {unreachable!("Handle not round")}
    }
    path.add_rect(Rect::from_point_and_size((at.0 - radius/2., at.1 - radius/2.), (radius,  radius)), None);
    path.close();
    canvas.draw_path(&path, &paint);
    match kind {
        UIPointType::Point => {paint.set_color(stroke);},
        _ => {unreachable!("Handle not round")}
    }
    paint.set_style(PaintStyle::Stroke);
    canvas.draw_path(&path, &paint);
...
fn draw_point(at: (f32, f32), kind: UIPointType, handles: (Handle, Handle), selected: bool, canvas: &mut Canvas) {
    let mut paint = Paint::default();
    let factor = state.with(|v|v.borrow().factor);
    paint.set_anti_alias(true);
    paint.set_style(PaintStyle::StrokeAndFill);
    ...
    match handles {
        ...
        (Handle::Colocated, Handle::At(_, _)) | (Handle::At(_, _), Handle::Colocated) => {
            draw_square_point(at, kind, selected, if selected { SELECTED_STROKE } else { POINT_ONE_STROKE }, if selected { SELECTED_FILL } else { POINT_ONE_FILL }, canvas, &mut paint);
        },
        _ => {
            draw_square_point(at, kind, selected, if selected { SELECTED_STROKE } else { POINT_SQUARE_STROKE }, if selected { SELECTED_FILL } else { POINT_SQUARE_FILL }, canvas, &mut paint);
        }
    }
}
...
fn draw_handle(h: Handle, selected: bool, canvas: &mut Canvas) {
    match h {
        Handle::Colocated => {},
        Handle::At(x, y) => {
            draw_point((calc_x(x), calc_y(y)), UIPointType::Handle(HandleStyle::Floating), (Handle::At(0., 0.), Handle::At(0., 0.)), selected, canvas);
        }
    }
}
...
fn draw_handlebars(a: Handle, b: Handle, at: (f32, f32), selected: bool, canvas: &mut Canvas) {
    ...
    paint.set_anti_alias(true);
    paint.set_color(if selected { SELECTED_FILL } else { HANDLEBAR_STROKE });
    paint.set_stroke_width(HANDLEBAR_THICKNESS * (1./factor));
    paint.set_style(PaintStyle::Stroke);

    match a {
        Handle::At(x, y) => {path.move_to((calc_x(x), calc_y(y)));path.line_to((at.0, at.1));},
        _ => {path.move_to((at.0, at.1));}
    }
    match b {
        Handle::At(x, y) => {path.line_to((calc_x(x), calc_y(y)));},
        _ => {}
    }
    canvas.draw_path(&path, &paint);
}

pub fn draw_complete_point(point: &glifparser::Point, kind: UIPointType, selected: bool, canvas: &mut Canvas) {
    draw_handlebars(point.a, point.b, (calc_x(point.x), calc_y(point.y)), selected, canvas);
    draw_point((calc_x(point.x), calc_y(point.y)), UIPointType::Point, (point.a, point.b), selected, canvas);
