...
use glifparser::{Contour, Glif, Handle, MFEKGlif, Outline, Point, PointType, WhichHandle, glif::{HistoryEntry, HistoryType, Layer}};
        ...
        if let Some(clipboard) = &self.clipboard {
            ...
            let translated_outline: Outline<PointData> = outline.iter().map(|contour| {
                contour.iter().map(|point| {
                    ...
                    let offset_x = calc_x(self.mousepos.0 as f32) - size.width() as f32 / 2.;
                    let offset_y = calc_y(self.mousepos.1 as f32) - size.height() as f32 / 2.;
