...
use skia_safe::{Canvas, Paint, Path, Point, Rect, Font, Matrix};

use crate::{editor::Editor, user_interface::{Interface, MouseInfo}, tools::cut::{Cut, Intersection}, constants};
...
impl Measure {
    pub fn draw_line(&self, i: &Interface, v: &Editor, canvas: &Canvas, factor: f32) {
        ...
        self.draw_text(canvas, &format!("{:.2}", distance), distance_text_position, 0.0, 16., factor);
    
        // Display the angle beneath the horizontal line
        let text_position = vec2!(start_point.x + distance/2.0, start_point.y + 10.0);
        self.draw_text(canvas, &format!("{:.2}°", angle_degrees), text_position.into(),  0.0, 16., factor);
        ...
        if intersections.len() > 1 {
            ...
            let fcoords = (first_intersection.coords.0 as f32, first_intersection.coords.1  as f32);
            let lcoords = (last_intersection.coords.0  as f32, last_intersection.coords.1  as f32);
    ...
    fn draw_bracket(&self, canvas: &Canvas, start: (f32, f32), end: (f32, f32), offset: f32, factor: f32) {
        ...
        self.draw_text(canvas, &format!("{:.2}", distance), text_position, -rotation_angle, 12., factor);
    }
}
