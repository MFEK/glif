...
impl MoveVWSHandle {
    pub fn new(mirror: bool, constrain: bool, all: bool, handle: WhichHandle, mouse_info: MouseInfo) -> Self {
        MoveVWSHandle {
            mirror, 
            constrain,
            all,
    ...
    fn mouse_moved(&self, v: &mut Editor, _i: &Interface, mouse_info: MouseInfo) {
        ...
        let (normal_offset, tangent_offset) = mouse_coords_to_handle_space(v, mouse_info, self.handle);
