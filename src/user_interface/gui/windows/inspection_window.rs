                    ...
                    ui.collapsing("Point", |ui| {
                        ...
                        ui.collapsing("Handles", |ui| {
                            if point.has_handle(WhichHandle::A) {
                                ui.label("Handle A:");
                                if let Some(handle_a_pos) =
                                    point.get_handle_position(WhichHandle::A)
                                {
                                    point.set_handle_position(
                                        WhichHandle::A,
                                        egui_parsed_textfield(
                                            ui,
                                            "hax",
                                            handle_a_pos.0,
                                            &mut self.edit_buf,
                                ...
                                let prev_checked =
                                    point.get_handle(WhichHandle::A).unwrap() == Handle::Colocated;
                                let mut checked =
                                    point.get_handle(WhichHandle::A).unwrap() == Handle::Colocated;
                            ...
                            if point.has_handle(WhichHandle::B) {
                                ui.label("Handle B:");
                                if let Some(handle_b_pos) =
                                    point.get_handle_position(WhichHandle::B)
                                {
                                    point.set_handle_position(
                                        WhichHandle::B,
                                        egui_parsed_textfield(
...
fn point_equivalent<PD: PointData>(
    a: &dyn MFEKPointCommon<PD>,
    b: &dyn MFEKPointCommon<PD>,
) -> bool {
