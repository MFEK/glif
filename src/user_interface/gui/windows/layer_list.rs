            ...
            .show(ctx, |ui| {
                ...
                for layer in 0..layer_count {
                    ui.horizontal(|ui| {
                        ...
                        let eye_button = egui::Button::new(eye_con).min_size(egui::vec2(24., 0.));
                        ...
                        egui::popup::popup_above_or_below_widget(
                            ...
                            |ui| {
                                ...
                                let edit_response =
                                    ui.text_edit_singleline(self.edit_buf.get_buf("name", &name));
                                if edit_response.lost_focus() && self.popup_had_focus {
                                    if self.edit_buf.get_buf("name", &name).clone() != name {
                                        ...
                                        v.get_active_layer_mut().name =
                                            self.edit_buf.get_buf("name", &name).clone();
                                        v.end_modification();
                                        v.set_active_layer(active_layer)
                                    }
                        ...
                        let current_operation =
                            v.with_glyph(|glif| glif.layers[layer].operation.clone());
