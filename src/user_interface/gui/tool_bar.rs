use egui::{Align2, Context, Ui, Stroke, Color32};

use crate::{editor::Editor, user_interface::Interface, tools::ToolEnum};

pub fn build_button(v: &mut Editor, ui: &mut Ui, text: &str, te: ToolEnum) {
    let stroke = if v.get_tool() == te {
        Stroke{ width: 2.0, color: Color32::from_rgb(9, 82, 128),}
    } else { Stroke::NONE };

    let button = egui::Button::new(text).stroke(stroke).min_size(egui::vec2(22., 15.));
