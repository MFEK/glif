use MFEKmath::{Piecewise, Vector, Evaluate};
use glifparser::{CapType, JoinType, VWSContour, WhichHandle, glif::{ContourOperations, InterpolationType, VWSHandle}};
...
pub fn mouse_coords_to_handle_space(v: &Editor, meta: MouseInfo, wh: WhichHandle) -> (f64, f64) {
    let (start_pos, tangent, _handle_pos) = get_vws_handle_pos(
        v,
...
pub fn set_vws_handle(
    ...
    v.with_active_layer_mut(|layer| {        
        ...
        let tangent_offset = if constrain { 0. } else { side_multiplier * tangent_offset };
        ...
        if mirror {
            ...
            vws_contour.handles[point_idx].tangent_offset =
                tangent_offset;
        }
    });
...
pub fn set_all_vws_handles(
    ...
    let contour_idx = v.contour_idx.unwrap();
    let mut vws_contour = get_vws_contour(v, contour_idx).unwrap_or(generate_vws_contour(v, contour_idx));
...
pub fn get_vws_handle_pos(v: &Editor, contour_idx: usize, handle_idx: usize, side: WhichHandle) -> (Vector, Vector, Vector) {
    v.with_active_layer(|layer| {
        let vws_contour = get_vws_contour(v, contour_idx).unwrap_or(generate_vws_contour(v, contour_idx));
        let contour_pw = Piecewise::from(&get_contour!(layer, contour_idx));
...
pub fn clicked_handle(v: &Editor, i: &Interface, meta: MouseInfo) -> Option<(usize, usize, WhichHandle)> {
