...
impl Guidelines {
    pub fn tool_dialog(&mut self, v: &mut Editor, i: &mut Interface, ui: &imgui::Ui) {    
        ...
        .build(ui, || {
            ...
            let guideline_count = v.with_glyph(|glif| {
                glif.guidelines.len()
            });
            ...
            for guideline in 0 .. guideline_count {
                ...
                if ui.is_item_clicked(imgui::MouseButton::Left) {
                    i.push_prompt(InputPrompt::Text {
                        ...
                        default: v.with_glyph(|glyph| glyph.guidelines[guideline].name.clone().unwrap_or("".to_string())),
                        func: Rc::new(move |editor, string| {
                            editor.with_glyph_mut(|glyph| glyph.guidelines[guideline].name = Some(string.clone()));
                        }),
                    });
                }
